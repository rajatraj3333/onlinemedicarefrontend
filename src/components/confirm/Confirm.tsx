...
import { DoctorDetails } from "../registration/types";
function Confirm() {
  ...
  const [bookedslot, setbookslot] = useState<Object[]>();
...
d.setUTCMonth(month)
  ...
  useEffect(() => {
    ...
    api.post("/doctor/bookslottime", data).then((res) => {
      setdataload(true);
        setbookslot(res.data.data.map((item:{slottime:string}) => item.slottime));
    });
  }, [date]);
  ...
  let confirmBtndisabled:number = mode === "" ?1:0
      ...
      <Loader data={dataload}>
        <div className="confirm">
          ...
          <div className="confirmbtn">
            <button
              className="cnbtn"
              disabled={confirmBtndisabled?true:false}
              onClick={confirm}
            >
              Confirm
            </button>
