...
function Edit({ isopen, setopen, data, updatesave }:{isopen:boolean,setopen:(t:boolean)=>void,data:any,updatesave:(data:any)=>void}) {
  const Option = { Select };
        ...
        <fieldset style={{ padding: "10px", marginTop: "30px" }}>
          ...
          <div className="selectanddate">
            ...
            <DatePicker
              ...
              onChange={(date) =>
                setlastdate(moment(new Date(date).getTime()).format("DD-MM-YYYY"))
              }
              format={"DD-MM-YYYY"}
...
function Doctor() {
  ...
  const edit = (id, type = "edit", e:React.ChangeEvent<HTMLInputElement>|null = null) => {
    const findDoctorIndex = doctorlist.findIndex((item:any) => item.id === id);
